...
export function DailyPayments() {
  ...
  useEffect(() => {
    const list = [
      ...cashTransactionService.getByType("payment"),
      ...cashTransactionService.getByType("exchange"),
  ...
  return (
    <div className="rounded-lg bg-white/5 p-4">
      <h2 className="mb-4 text-lg font-bold text-red-400">المدفوعات اليومية</h2>

      <div className="mb-4 rounded border border-red-500/30 bg-red-500/10 p-3 text-center">
        <span className="text-sm text-gray-400">الإجمالي الكلي: </span>
        <span className="text-xl font-bold text-red-400" dir="ltr">
          {grandTotal.toLocaleString("en-US", { minimumFractionDigits: 2 })}
        </span>
      </div>

      {grouped.length === 0 ? (
        <div className="py-8 text-center text-sm text-gray-400">لا توجد مدفوعات</div>
      ) : (
          ...
          return (
            <div key={date} className="mb-4 rounded border border-white/10">
              <div className="flex items-center justify-between bg-white/5 px-4 py-2">
                ...
                <span className="text-sm font-bold text-red-400" dir="ltr">
                ...
                <tbody>
                  {items.map((t) => (
                    <tr key={t.id} className="border-b border-white/5 hover:bg-white/5">
                      <td className="px-3 py-1.5 text-sm text-gray-400">{t.type === "payment" ? "دفع" : "صرف"}</td>
