...
export function PaymentsReport() {
  ...
  useEffect(() => {
    let list = cashTransactionService.getByType("payment");
  ...
  return (
    <div className="rounded-lg bg-white/5 p-4">
      <h2 className="mb-4 text-lg font-bold text-red-400">عمليات الدفع</h2>
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          ...
          <tbody>
            {txns.length === 0 ? (
              <tr><td colSpan={6} className="px-3 py-4 text-center text-sm text-gray-400">لا توجد عمليات دفع</td></tr>
            ) : (
              txns.map((t) => (
