...
export function SalesProfitReport() {
  ...
  const rows = useMemo(() => profitReportsService.getSalesProfit(fromDate || undefined, toDate || undefined), [fromDate, toDate]);
  ...
  const summary = useMemo(() => {
    ...
    return { totalRevenue, totalCost, totalProfit, margin: Math.round(margin * 100) / 100 };
  }, [rows]);

  return (
    <div className="rounded-lg bg-white/5 p-4">
      <h2 className="mb-4 text-lg font-bold text-rose-400">أرباح المبيعات</h2>
      ...
      <div className="mb-4 grid grid-cols-4 gap-4">
        ...
        <div className="rounded bg-amber-500/10 p-3 text-center">
          ...
          <div className="text-lg font-bold text-amber-400" dir="ltr">{summary.margin}%</div>
        </div>
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          ...
          <tbody>
            {rows.length === 0 ? (
              <tr><td colSpan={5} className="px-3 py-4 text-center text-sm text-gray-400">لا توجد بيانات أرباح</td></tr>
            ) : (
              rows.map((r, idx) => (
