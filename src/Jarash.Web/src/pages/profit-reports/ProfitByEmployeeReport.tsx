...
export function ProfitByEmployeeReport() {
  const rows = useMemo(() => profitReportsService.getProfitByEmployee(), []);
  ...
  return (
    <div className="rounded-lg bg-white/5 p-4">
      <h2 className="mb-4 text-lg font-bold text-teal-400">الأرباح حسب الموظف</h2>
      <p className="mb-4 text-sm text-gray-400">تحليل الإيرادات المحققة حسب مقدم الخدمة</p>

      <div className="mb-4 grid grid-cols-3 gap-4">
        <div className="rounded bg-white/5 p-3 text-center">
          <div className="text-xs text-gray-400">عدد مقدمي الخدمة</div>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-white/10 text-xs text-gray-400">
              <th className="px-3 py-2 text-right">مقدم الخدمة</th>
              <th className="px-3 py-2 text-left">عدد الخدمات</th>
          ...
          <tbody>
            {rows.length === 0 || (rows.length === 3 && rows.every((r) => r.revenue === 0)) ? (
              <tr><td colSpan={6} className="px-3 py-4 text-center text-sm text-gray-400">لا توجد خدمات مكتملة. قم بإضافة خدمات للغرف أولاً</td></tr>
            ) : (
              rows.map((r, idx) => (
