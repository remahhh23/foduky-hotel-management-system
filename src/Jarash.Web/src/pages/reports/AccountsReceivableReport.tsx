...
export function AccountsReceivableReport() {
  ...
  useEffect(() => {
    setBuckets(reportsService.getAccountsReceivable());
  }, []);
  ...
  return (
    <div className="rounded-lg bg-white/5 p-4">
      <h2 className="mb-4 text-lg font-bold text-teal-400">الذمم المدينة</h2>
      <p className="mb-4 text-sm text-gray-400">الفواتير غير المسددة للعملاء (النزلاء)</p>

      <div className="grid grid-cols-4 gap-4">
        {buckets.map((bucket) => (
          <div key={bucket.label} className="rounded bg-white/5 p-3">
            <h3 className="mb-2 text-sm font-semibold text-orange-400">{bucket.label}</h3>
      ...
      <div className="mt-4 rounded border border-teal-500/30 bg-teal-500/10 p-3 text-center text-sm text-teal-300">
        إجمالي الذمم المدينة: {grandTotal.toLocaleString("en-US", { minimumFractionDigits: 2 })}
      </div>
    </div>
  );
}
